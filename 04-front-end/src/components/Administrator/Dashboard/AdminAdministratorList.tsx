...
export default function AdminAdministratorList(){
    ...
    function AdminAdministratorRow(props: IAdministratorRowProperties){
        ...
        const [ newUsername, setNewUsername ] = useState<string>(props.administrator.username);
        ...
        function doToggleAdministratorActiveState(){
            api("put", "/api/administrator/"+props.administrator.administratorId, {isActive: !props.administrator.isActive,})
            .then(res => {
                if(res.status === 'error'){
            ...
            .catch(error => {
                if (error.response) {
                  // Handle HTTP errors (e.g., response status codes outside of the 2xx range)
        ...
        function doChangePassword(){
            api("put", "/api/administrator/"+props.administrator.administratorId, {password: newPassword,})
            .then(res => {
                if(res.status === 'error'){
                    return setErrorMessage(res.data.message || "Password must be at least 8 character long with one Capital letter, number and special char!")
                }
                setAdministrators(res.data);
            });
        }
        function doEditUsernameAndEmail(){
            api("put", "/api/administrator/"+props.administrator.administratorId, {username: newUsername, email: newEmail,})
            .then(res => {
                if(res.status === 'error'){
                    return setErrorMessage(res.data.message || "Username or email already exists!")
                }
                else{
            ...
            return (
                ...
                <td>
                    ...
                    <div className="input-group">
                        <button className="btn btn-primary btn=sm" onClick={() => setEditNamePartsVisible(true)}>Edit</button>
                    </div>}
                    { editNamePartsVisible && <div>
                        <div className="form-group mb-3">
                            <input type="text" className="form-control form-control-sm"value={newUsername} onChange={ e => setNewUsername(e.target.value)} />
                        </div>
                        <div className="form-group mb-3">
                            <input type="text" className="form-control form-control-sm" value={newEmail} onChange={ e => setNewEmail(e.target.value)} />
                        </div>
                        { (newUsername !== props.administrator.username || newEmail !== props.administrator.email) &&
                        <button className="btn btn-sm btn-primary" onClick={() => doEditUsernameAndEmail()}>Save</button>
                        }
                        <button className="btn btn-sm btn-danger" onClick={() => {setNewUsername(props.administrator.username); setNewEmail(props.administrator.email); setEditNamePartsVisible(false)}}>Cancel</button>
                ...
                <div className={"btn btn-sm " + activeSideClass}><FontAwesomeIcon icon={ faSquareCheck }/>
                </div>
                <div className={"btn btn-sm " + inactiveSideClass}><FontAwesomeIcon icon={ faSquare}/>
                </div>
                </div>
                </td>
                <td>
        ...
        <div>
            ...
            <table className="table table-sm table-hover admin-list">
                ...
                <tbody>
                    ...
                    {administrators.map(admin => <AdminAdministratorRow key={"administrator" + admin.administratorId} administrator={admin} onAdministratorUpdated={loadAdministrators} />)}
