...
class PageController extends BaseController {
    ...
    async getAllPages(req: Request, res: Response) {
      this.services.page.getAll(DefaultCategoryAdapterOptions)
      .then(result => {
        if (result.length === 0) {
    ...
    async getPageById(req: Request, res: Response) {
        const pageId: number = +req.params?.pid;
            ...
            this.services.page.getById(pageId, {DefaultCategoryAdapterOptions})
              .then(result => {
    ...
    async addPage(req: Request, res: Response) {
      ...
      async deletePage(req: Request, res: Response) {
        const pageId: number = +req.params?.pid;
        this.services.page.getById(pageId, {})
          .then(result => {
            if (result === null) {
